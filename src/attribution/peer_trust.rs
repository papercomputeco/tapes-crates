//! Whether the process on the other end of a connection is the harness the
//! capture client launched.
//!
//! # Why this exists
//!
//! A harness that stamps its own `X-Tapes-*` envelope hands a capture proxy a
//! session identity the proxy cannot otherwise discover. Believing that
//! envelope because the *launched* harness was self-attributing is not enough:
//! it says nothing about who sent the request. A capture proxy listens on
//! loopback, and every process on the machine can reach a loopback port — an
//! ephemeral port number is obscurity, not a boundary. Without this check any
//! local process could stamp two headers and have a turn persisted, and a
//! session link announced, under a session id of its choosing.
//!
...
//! socket" (see [`super::peer_pid`]), and the process tree answers "is that
//! PID ours".
//!
//! This lives in the shared crate for the same reason [`super::peer_pid`]
...
//! # Not the whole check
//!
//! Ancestry alone cannot exclude the harness's *own* subprocesses: a command
//! the harness runs in a shell tool is a descendant of the launched PID and
//! would pass this walk while forging another session's envelope. Consumers
//! close that hole with a per-launch secret — generated by the launcher,
//! handed to the in-harness extension via [`crate::plugin::GATEWAY_NONCE_ENV`],
//! echoed back in [`crate::plugin::GATEWAY_NONCE_HEADER`], and required
//! alongside this ancestry check before an envelope is believed.
//!
...
//! process the capture client started.
//!
//! # Failing closed
//!
...
/// The parent PID of `pid`, or `None` when it cannot be determined.
///
/// macOS has no `/proc`; `proc_pidinfo` is the supported query, and
/// [`super::codex::process`] reaches for the same call for its own per-PID
/// lookups.
#[cfg(target_os = "macos")]
fn parent_of(pid: i32) -> Option<i32> {
    use std::mem::{MaybeUninit, size_of};

    // Not exported by `libc`, so it is spelled here the way
    // [`super::codex::process`] spells the flavours it needs. From
    // `<sys/proc_info.h>`.
...
mod tests {
    ...
    fn a_process_is_its_own_launched_harness() {
        // The common case on an `exec`ing wrapper chain: the PID the capture
        // client spawned is the PID that opens the socket.

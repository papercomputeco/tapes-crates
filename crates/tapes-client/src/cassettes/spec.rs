...
//! *back*, which a consumer prints verbatim without modelling. So
...
///
/// Extracted from tapesctl, where the reserved list was a compile-time
/// constant; each consumer's subcommands define their own flags, so the list
...
/// surface feeds it a runtime-discovered document, and tapesctl's core surface
/// feeds it a vendored contract. One reducer, two document sources — so the
/// two surfaces cannot read OpenAPI differently.
#[must_use]
...
mod tests {
    ...
    /// moved test bodies read exactly as they did before the extraction.

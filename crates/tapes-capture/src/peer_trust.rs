...
//! socket" (see [`crate::peer_pid`]), and the process tree answers "is that
//! PID ours".
//!
...
//! handed to the in-harness extension via [`crate::gateway::GATEWAY_NONCE_ENV`],
//! echoed back in [`crate::gateway::GATEWAY_NONCE_HEADER`], and required
...
/// the codex process lookup reaches for the same call for its own per-PID
/// lookups.
#[cfg(target_os = "macos")]
fn parent_of(pid: i32) -> Option<i32> {
    ...
    // the codex process lookup spells the flavours it needs. From
    // `<sys/proc_info.h>`.
